...
The Table is built from compositional primitives that map directly to HTML table elements. Each primitive is a styled wrapper that accepts all native HTML attributes, giving you full control over layout, sizing, and responsiveness.
...
Rows accept an `onClick` handler, which enables interaction styling.
...
In the default `auto` layout, use `style={{ width: "1%" }}` to minimize a column to fit its content. This is useful for action columns or icon-only columns.
