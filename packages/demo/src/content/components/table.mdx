...
Rows accept an `onClick` handler, which enables hover interactions. Use `data-state="selected"` to mark a row as selected — try clicking a row below to toggle its selected state.
