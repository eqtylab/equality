...
The syntax palette is defined in CSS and resolves against the nearest theme root, so a code block follows whatever theme surrounds it with no configuration. That holds inside a shadow root, and with more than one theme on a page.

To restyle it, override the `--syntax-*` custom properties on the code block or on any ancestor.
