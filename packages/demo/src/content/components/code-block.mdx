...
The syntax palette follows the surrounding theme as set by `<ThemeProvider />`. Unlike the rest of the library, which propagates themes purely through CSS, `<CodeBlock>` resolves the theme in JavaScript, so the provider needs to be told which theme it is on whenever the page-wide value would be wrong. That happens inside a shadow root, where the page-wide lookup can't reach, and when more than one theme is on screen at once.

See [Theme-aware components](/getting-started/usage#theme-aware-components) for how to set it.
