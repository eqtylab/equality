...
Tooltip shows a small hint anchored to an element when the user hovers or focuses it — a label for an icon button, or a brief explanation of a control. It's built on [Radix UI Tooltip](https://www.radix-ui.com/primitives/docs/components/tooltip), so it opens on hover and keyboard focus, dismisses on blur or <kbd>Esc</kbd>, and includes a pointer arrow.

Keep tooltips short and non-essential. For rich or interactive overlays use a [Popover](popover). The content renders in a portal, so it escapes overflow clipping.
