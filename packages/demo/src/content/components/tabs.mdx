...
| `defaultValue`       | `string`                    | First item      | The value of the tab that should be active by default. Defaults to the first item's value if not specified.                               |
...
| `onValueChange`      | `(value: string) => void`   | —               | Callback function called when the active tab changes. Receives the new tab's value as an argument.                                        |
