...
heading: "Format Date"
description: "Render a date as absolute, relative, or an exact countdown using a semantic <time> element"
---

import { FormatDate } from "@eqtylab/equality";
import {
  FormatDateCountdownDemo,
  FormatDateIsoDemo,
...
The <code>FormatDate</code> component renders a date in one of three ways: **absolute** time (e.g. "Jun 09 2026, 18:42:03 UTC"), **relative** time (e.g. "2 weeks ago", "Just now"), or a **countdown** to a future date ("10 days"). It always renders as a semantic HTML [`<time>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/time) element with a `dateTime` attribute.
...
<FormatDateCountdownDemo client:only="react" />
...
| `displayAs`       | How to render the date                                                  | `absolute`, `relative`, `countdown` | `absolute` | ❌       |
