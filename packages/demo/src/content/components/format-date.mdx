...
The <code>FormatDate</code> component renders a date in one of four ways: **absolute** time (e.g. "Jun 09 2026, 18:42:03 UTC"), **relative** time (e.g. "2 weeks ago", "Just now"), or an exact countdown with **until** ("10 days") and **since** ("10 days ago"). It always renders as a semantic HTML [`<time>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/time) element with a `dateTime` attribute.
...
`until` omits the preposition so it reads under a label ("Expires In: 10 days"), while `since` appends "ago" so it stands alone ("Last observed: 10 days ago").
...
Set `displayAs="until"` to count down to a future date in its largest whole unit. Days stay exact out to three months, after which the value coarsens to months and then years. A date that has already passed renders the `---` placeholder.
...
Set `displayAs="since"` for the same exact measurement in the other direction, with "ago" appended. A date that has not yet arrived renders the `---` placeholder.
