...
Whichever message is showing is wired to the input with `aria-describedby`, and the slot is a polite live region, so a screen reader reads the error when it appears. Do not repeat the error in a separate element — that double-announces it.
