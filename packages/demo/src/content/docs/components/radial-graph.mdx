...
By default the label displays the rounded percentage, but you can override it with any text. The `variant` prop ties the color to a status (success, danger, warning), so the value's meaning reads at a glance. For a simple linear bar, use [Progress](/components/progress) instead.
