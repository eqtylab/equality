...
Its open state is fully controlled through the `isOpen` prop — pair it with your own trigger (such as a [Button](/components/button)) that toggles the state. Use it for show/more panels, expandable rows, and inline disclosure sections. It is built on [Motion](https://motion.dev/), so the animation respects the transition timing defined by the component.
