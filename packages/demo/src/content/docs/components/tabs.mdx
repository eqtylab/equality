...
| `staticRender`       | Render for a page where React never runs on the client                   | `boolean`                 | `false`       | ❌       |

### Statically Rendered Tabs

`staticRender` is for when the component must work statically without client-side rendering, such as in `@eqtylab/docs`. Leave the prop off for ordinary interactive use.
