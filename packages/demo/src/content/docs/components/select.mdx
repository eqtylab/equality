...
<kbd>Enter</kbd> in the search box selects the first matching option.
<kbd>↓</kbd> moves from the search box to the first option and <kbd>↑</kbd> to
the last; <kbd>↑</kbd> on the first option returns to the search box. Typing or
