...
The menu widens to fit its longest option, up to `max-w-80`. Each option stays on one line, so a label longer than that ends in an ellipsis, with the full label in its tooltip.
