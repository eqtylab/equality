...
For single-line input use an [Input](/components/input); pair the textarea with a [Label](/components/label) via a shared `id`/`htmlFor`, or use it inside a [Form](/components/form) field for validation and accessible wiring.
