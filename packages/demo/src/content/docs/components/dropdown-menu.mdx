...
Generally using `DropdownMenuSearch` over the default typeahead is encouraged for _most_ dropdown menus in our apps.
...
<DropdownMenuDemo client:only="react" variant="with-search-reveal" />
...
Mark an item `persistent` to keep it on screen while searching, such as a footer action that belongs to the whole list. It is never a result: it doesn't count towards `DropdownMenuEmpty` or the announced result count, and <kbd>Enter</kbd> in the search box skips it. A `persistent` separator likewise stays visible.

To act on only what the search shows, read it with `useDropdownMenuSearchQuery` from any component inside the menu. `matches` applies the same rule the items use.

<DropdownMenuDemo client:only="react" variant="with-search-persistent" />
...
// Inside the menu, so it can read the search
const ColumnFooter = ({ onShow }) => {
  const { matches } = useDropdownMenuSearchQuery();
  const shown = COLUMNS.filter((column) => matches(column.label)).map(
    (column) => column.key,
  );
...
<DropdownMenuContent align="start">
  ...
  {COLUMNS.map(({ key, label }) => (
    <DropdownMenuCheckboxItem
      key={key}
      checked={visible[key]}
      onCheckedChange={(checked) => setVisible({ ...visible, [key]: checked })}
      onSelect={(event) => event.preventDefault()}
    >
      {label}
    </DropdownMenuCheckboxItem>
...
<kbd>Enter</kbd> in the search box activates the first matching item, skipping
`persistent` ones.
<kbd>↓</kbd> moves from the search box to the first item and <kbd>↑</kbd> to the
last; <kbd>↑</kbd> on the first item returns to the search box. Typing or
<kbd>Backspace</kbd> while an item is focused keeps editing the query.
...
To pick a value in a form rather than run an action, use a [Select with search](/components/select#search--filtering).
...
Call from any component inside a `DropdownMenu`. Returns the current search state:

| Name          | Description                                                | Type                             |
| ------------- | ---------------------------------------------------------- | -------------------------------- |
| `query`       | The query as typed                                         | `string`                         |
| `isSearching` | Whether a non-blank query is active                        | `boolean`                        |
| `matches`     | Whether some text matches the query, by the rule items use | `(textValue: string) => boolean` |
