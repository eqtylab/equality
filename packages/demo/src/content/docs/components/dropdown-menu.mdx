...
`useDropdownMenuSearchQuery()` only works inside a `DropdownMenu` (it reads the search via context) and throws otherwise.
...
last; <kbd>↑</kbd> on the first item and <kbd>↓</kbd> on the last return to the
