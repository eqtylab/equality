...
The `label` is shown on the trigger before anything is selected, and as the heading inside the menu. Once an option is selected, its label replaces the trigger text. With `searchable`, the search box replaces the heading and takes `label` as its accessible name.
...
The search box sits at the top of the menu and is focused as soon as it opens. Options that don't match hide themselves, matching against each option's `label`. <kbd>Enter</kbd> chooses the first match.
...
| ------------------- | ---------------------------------------------------------------------------- | ---------------------------------------------------- | ------------------- | -------- |
| `label`             | Trigger text before selection; the menu heading, or the search box's name.   | `string`                                             | —                   | ✅       |
