...
Info Card surfaces a single labelled piece of information alongside an icon — for example a metadata field, a setting, or a summary value. It composes a [Card](/components/card) with an [Icon](/components/icon) and a label/description pair, giving these small facts a consistent treatment across the interface.

The `description` accepts any node, so it can hold plain text, a number, or richer content such as a [Badge](/components/badge). Passing an `onClick` makes the whole card interactive (inheriting Card's hover and click affordance), which is useful when the card links somewhere.
...
The `icon` accepts a [Lucide](https://lucide.dev/icons/) icon name or a React element, matching the [Icon](/components/icon) component.
...
Passing an `onClick` handler makes the whole card clickable and adds a hover affordance, inheriting the behavior from [Card](/components/card). Use it when the card links somewhere or triggers an action. Click the card below to see it respond.
