...
Associate a label with a control in one of two ways: give the label an `htmlFor` prop matching the control's `id`, or nest the control inside the label. For fields built with [Form](/components/form), use `FormLabel` instead — it wires up this association automatically.
