...
Use it for rich, interactive overlays — filters, pickers, small forms, or menus of actions. For simple hover hints use a [Tooltip](/components/tooltip); for a modal use a [Dialog](/components/dialog). The content is rendered in a portal, so it escapes overflow clipping from ancestors.
...
Wrap a trigger and content in `Popover`. Use `asChild` on the trigger to render your own element (such as a [Button](/components/button)) as the trigger:
