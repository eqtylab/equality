...
It is a compound component: a `RadioGroup` wraps a set of `RadioGroupItem`s. Pair each item with a [Label](/components/label) so the choice has a visible, clickable caption. Use it for a handful of always-visible options; when space is tight or the list is long, prefer a [Select](/components/select) or [Radio Dropdown](/components/radio-dropdown).
