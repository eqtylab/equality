...
The Spinner component provides a visual loading indicator to communicate that content or an action is in progress. Use it when fetching data, submitting forms, or during any asynchronous operation where the user should wait. For a full-screen blocking indicator, use [Loading Overlay](/components/loading-overlay).
