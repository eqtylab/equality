...
To pick a value in a form, use a [Select with search](/components/select#search--filtering) instead of wrapping Command in a Popover.
...
Wrap the command list in `CommandDialog` to present it as a modal command palette — commonly opened with a keyboard shortcut such as <kbd>⌘K</kbd>. `CommandDialog` accepts the same `open`/`onOpenChange` props as [Dialog](/components/dialog), plus an optional `title` and `description` (visually part of the dialog header and read by screen readers).
