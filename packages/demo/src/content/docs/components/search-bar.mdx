...
Search Bar is a purpose-built [Input](/components/input) for searching and filtering. It shows a leading search icon and, once text is entered, a trailing clear button that empties the field. It renders inside a semantic `<search>` landmark, so assistive technology can identify it as the page's search region.
