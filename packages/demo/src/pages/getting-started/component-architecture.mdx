...
At the root level, the theme color tokens are imported via the `@eqtylab/equality/theme-colors.css` import or the `<ThemeProvider>`:
...
This setup means host apps can import components without any Tailwind setup of their own if they like.
