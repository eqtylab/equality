...
Components have no `theme` prop of their own — they all take it from the provider they sit inside, so a subtree can never disagree with itself.

Where a second `<ThemeProvider />` is too much — it brings the scoped theme CSS and an unstyled wrapper element with it, which a host's layout may not survive — use `<ThemeScopeProvider />` instead. It declares the theme and nothing else: no styles, no DOM.
...
Reach for `<ThemeProvider />` in an app you own, and this only where you can't.
